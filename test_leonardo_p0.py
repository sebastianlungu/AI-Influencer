...
print(f"Input Size: {settings.leonardo_width}×{settings.leonardo_height}")
print(f"Alchemy: {settings.leonardo_use_alchemy}, Preset: {settings.leonardo_preset_style}")
if settings.leonardo_use_alchemy:
    expected_w = int(settings.leonardo_width * 1.75)
    expected_h = int(settings.leonardo_height * 1.75)
    print(f"Expected Output (1.75x): {expected_w}×{expected_h}")
print(f"Steps: {settings.leonardo_steps}, CFG: {settings.leonardo_cfg}")
print(f"Legacy Params: {settings.leonardo_use_legacy_params}")
...
try:
    ...
    print("  requested=(model=... size=864x1536 alchemy=true preset=DYNAMIC lora=155265@0.80)")
    print("  applied=(alchemy=true preset=DYNAMIC lora_present=true) gen_id=... path=...")
    print("\nExpected result: Image should show caramel-blonde hair (Eva Joy LoRA applied)")
    print("If brunette appears, LoRA was ignored (test designed to detect this)")
    print("=" * 80)
